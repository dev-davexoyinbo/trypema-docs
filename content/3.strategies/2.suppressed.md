...
Full suppression is controlled by `hard_limit_factor`. It is a validated newtype (`HardLimitFactor`): `HardLimitFactor::try_from(value)` fails if `value < 1.0`. Default is `1.0`.
...
- With `hard_limit_factor = 1.5`: 50% burst headroom. Suppression gradually ramps from 0 to 1 over the range from `rate_limit` to `rate_limit * 1.5`.
- With `hard_limit_factor = 2.0`: 100% burst headroom.
