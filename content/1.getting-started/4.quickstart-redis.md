...
The suppressed Redis strategy behaves like the local one: it returns
`Suppressed { is_allowed, suppression_factor }` near/over capacity.

As load increases beyond the hard limit, suppression ramps up to full suppression
(`suppression_factor = 1.0`, `is_allowed = false`).
...
::warning
Redis operations are implemented as atomic Lua scripts.

For the absolute strategy, the admission check and increment happen inside the same script (`inc(...)`), so concurrent callers do not overshoot due to races.
