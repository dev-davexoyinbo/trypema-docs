...
::note
Local state is per-process. If you run multiple instances and need shared limits, use the Redis provider (strict) or the Hybrid provider (higher throughput with bounded state lag).
::
