...
replace it. A matched `set_if(...)` or `set_if_preserve_history(...)` recomputes and replaces the
stored capacity; a matched zero target removes the key.
...
- Treat the first increment's computed capacity as sticky until a matched conditional update or
  key removal.
