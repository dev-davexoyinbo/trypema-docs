...
let prefix = RedisKey::try_from("billing-api")?;
let key = RedisKey::try_from("tenant_42")?;
```

The provider prefix isolates one application's Redis state; the operation key identifies the
subject inside that namespace. Instances that must share limits need the same prefix and settings.
