...
pub enum RateLimitDecision {
    ...
    Rejected {
        ...
        /// Estimate of how many requests will still be counted in the
        /// window after `retry_after_ms` elapses (total - oldest_bucket_count).
...
- `remaining_after_waiting`: an estimate of how many requests will still be counted in the window after `retry_after_ms` elapses (i.e., after the oldest bucket expires).
...
- `remaining_after_waiting` is computed as `total_count - oldest_bucket_count`: the number of requests that will remain in the window once the oldest bucket expires. It may be `0` if all activity is heavily coalesced into the oldest bucket.
...
- If suppression declines a call (`is_allowed == false`), the call is recorded as declined.
- You can derive accepted usage as `observed_total - declined_total`.
- As observed usage approaches/exceeds the hard limit, suppression increases up to full suppression
...
### Suppressed strategy (`Allowed` / `Suppressed`)
